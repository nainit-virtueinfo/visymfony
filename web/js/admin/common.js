...
function checkUncheckAllApprove(frm,checkname,allcheckbox)
{// JavaScript Document
   if(typeof checkname == "undefined")
   {
		  var checkname='approveIds[]';
   }
	
	 if(typeof allcheckbox =="undefined")
	 {
		var allcheckbox="approveId"; 
	 }
    for ( x=0; x<frm.elements.length; x++ )
	{
		var e = frm.elements[x];
...
function checkUncheckOneApprove(frm,checkname,allcheckbox)
{
	var countElement = 0;
	var selectedElement = 0;
...
function approveAll(frm, _ttl, _id, _msgconfirm, _msgselect)
{
	...
	for ( x=0; x<frm.elements.length; x++ )
	{
		...
		if ( e.name == 'approveIds'+_id+'[]' && e.checked ) 
			selectedElement++;
	}
...
function checkOption(id,formname)
{
	if(id=='-1')
	{

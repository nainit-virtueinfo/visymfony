...
echo $oAppCommon->oForm->renderFormTag(url_for($ssSaveRedirectLink.(!$oAppCommon->oForm->getObject()->isNew() ? '?'.$oAppCommon->ssBaseId.'='.$oFormObjectData[$oAppCommon->ssBaseId] : '' ) ),array('id'=>'frmAddEdit','name'=>'frmAddEdit'));
        ...
        <?php endif;
            ...
            foreach($oAppCommon->oForm as $ssFieldName=>$oFormdata): 
              if($ssFieldName=='country')
               {?>
                <tr class="<?php echo ($snCnt%2==0) ? "odd" : "even" ?>"  id="tbRow<?php echo $snCnt++;?>" align="left" >
                    ...
                    <td width="85%" align="left" id="tbRowCell<?php echo $snTabindex;?>2" >
                        <div class="fleft">
                          <?php echo $oAppCommon->oForm['country']->render(array('onchange'=>jq_remote_function(array('update'=>'company_state', 'url'=>'/company/getState?ssFormName='.$ssFormName, 'method'=>'post', 'with'=>"'&id=' + this.options[this.selectedIndex].value",))))?>
                        </div>
                ...
                <tr class="<?php echo ($snCnt%2==0) ? "odd" : "even" ?>"  id="tbRow<?php echo $snCnt++;?>" align="left" >
                    ...
                    <td width="85%" align="left" id="tbRowCell<?php echo $snTabindex;?>2" >
                        <div id="company_state" class="fleft">
                          <?php $ssPara = sfcontext::getInstance()->getRequest()->getParameter($ssFormName);
                                echo $oAppCommon->oForm['state']->render(array('onchange'=>'checkOption(this.options[this.selectedIndex].value,"company_state_text")'))?>
                            <?php if($ssPara['state'] == '-1' && $oAppCommon->oForm->getObject()->isNew()){?>
                            <div id='company_state_text' style="display:block;width:200px;">
                                <?php echo input_tag($ssFormName.'[otherState]',$ssPara['otherState']); ?>
                            </div>
                            <?php }?>
                            <?php if(!is_numeric($ssPostState) && !$oAppCommon->oForm->getObject()->isNew()){?>
                            <div id='company_state_text' style="display:block;width:200px;">
                                <script type="text/javascript">
                                    document.frmAddEdit.company_state.value = -1;
                                </script>
                                <?php echo input_tag($ssFormName.'[otherState]',$ssPostState); ?>
                            </div>
                            <?php }?>
                            <?php if(isset($ssFormName) && is_numeric($ssPostState)  || (isset($ssFormName) && $ssPostState =='')){?>
               ...
               if($oAppCommon->oForm->getWidget($ssFieldName)->getAttribute('type')!='hidden' && $ssFieldName != 'country' && $ssFieldName != 'state' && count($oAppCommon->oForm[$ssFieldName])==1 && !isset($oEmbedeForms[$ssFieldName])):?>
...
?>								</span>
			 ...
			 <td>&nbsp;</td>
                <td align="left" colspan="2">
                	<div class="clearb">
                        <div class="menu clearb">
							<input type="submit" title="<?php echo __('Save');?>" tabindex="17" value="<?php echo __('Save');?>" name="Save"/>
						</div>
                        <div class="menu padding0" >
							<?php echo link_to_function(__('Cancel'),"document.frmAddEdit.action='".url_for($oAppCommon->ssModuleName.'/cancel') ."';document.frmAddEdit.submit(); " ,array('tabindex'=>18,'title'=>__('Cancel'))); ?>                        
                        </div>
                    </div>
                </td>
              </tr>                
